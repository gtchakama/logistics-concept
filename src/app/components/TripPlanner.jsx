    ...
    const TripPlanner = () => {
        ...
        const vehicleColors = [
            'bg-indigo-100', 'bg-green-100', 'bg-yellow-100', 'bg-red-100', 'bg-purple-100',
    ...
    .map((passenger, index) => ({ passenger, originalIndex: index }))
    .filter(({ passenger }) =>
                                ...
                                <Droppable droppableId="passengerList">
                                    {(provided, snapshot) => (
                                        <ul
                                            ...
                                            className={`space-y-2 min-h-[200px] max-h-[400px] overflow-y-auto ${snapshot.isDraggingOver ? 'bg-gray-200' : ''
                                                }`}
                                        >
                                            {filteredPassengers.map(({ passenger, originalIndex }) => (
    <Draggable key={passenger} draggableId={passenger} index={originalIndex}>
        {(provided, snapshot) => (
        <li
            ref={provided.innerRef}
            {...provided.draggableProps}
            {...provided.dragHandleProps}
            className={`p-3 bg-gray-50 rounded-md cursor-pointer text-gray-800 transition-all duration-200 hover:bg-indigo-100
            ${selectedPassenger === passenger ? 'bg-indigo-200 border-2 border-indigo-500' : 'border-2 border-gray-100'}
        ...
        >
            ...
            <span className="text-gray-400 text-sm">(Drag)</span>
                        ...
                        <div className="col-span-2">
                            <h2 className="text-2xl font-semibold mt-4 text-gray-800">Vehicles</h2>
                            <div className="mb-4 p-4 bg-white rounded-lg shadow-md">
                                ...
                                <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2">
                                    ...
                                    <button
                                        ...
                                        className="w-full md:w-auto px-4 py-2 bg-indigo-500 text-white rounded-md hover:bg-indigo-600 transition-colors duration-200"
                                    >
                                        Add Vehicle
                                    </button>
                                </div>
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                        <div className="bg-white p-6 rounded-lg">
                            ...
                            <div className="flex justify-end space-x-2">
                                ...
                                <button onClick={saveEditedVehicle} className="px-4 py-2 bg-indigo-500 text-white rounded-md">Save</button>
                            </div>
                        </div>
                    </div>
                )}
